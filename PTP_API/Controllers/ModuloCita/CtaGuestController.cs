...
namespace PTP_API.Controllers.ModuloCita
{
    ...
    public class CtaGuestController : ControllerBase
    {
        ...
        [SwaggerOperation(Summary = "Obtener Invitados de Citas", Description = "Devuelve una lista de Invitados de Citas o un invitado específico si se proporciona un ID")]
        public async Task<IActionResult> GetAllGuest([FromQuery] int? IdMeetingPlace, long? companyId)
        {
            try
            {
                if (IdMeetingPlace.HasValue)
                {
                    var place = await _guestService.GetByIdResponse(IdMeetingPlace.Value);
                    if (place == null)
                        return NotFound(Response<CtaGuestResponse>.NotFound("Invitado no encontrado."));

                    return Ok(Response<CtaGuestResponse>.Success(place, "Invitado encontrado."));
                }
                else
                {
                    var places = await _guestService.GetAllDto();
                    ...
                    return Ok(Response<IEnumerable<CtaGuestResponse>>.Success(
                        companyId != null ? places.Where(x => x.CompanyId == companyId).ToList() : places, "Invitados obtenidos correctamente."));
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> CreateGuest([FromBody] CtaGuestRequest guestDto)
        {
            try
            {
                ...
                return CreatedAtAction(nameof(GetAllGuest), Response<CtaGuestResponse>.Created(response));
            }
            catch (Exception ex)
            {
                return StatusCode(500, Response<string>.ServerError(ex.Message));
        ...
        public async Task<IActionResult> UpdateGuest(int id, [FromBody] CtaGuestRequest guestDto)
        {
            try
            {
                ...
                var existingPlace = await _guestService.GetByIdRequest(id);
