...
namespace PTP_API.Controllers.ModuloHelpDesk
{
    ...
    [Authorize]
    [EnableBitacora]
    public class HdkTicketsController : ControllerBase
    {
        private readonly IHdkTicketsService _ticketsService;
        private readonly IValidator<HdkTicketsRequest> _validator;

        public HdkTicketsController(IHdkTicketsService ticketsService, IValidator<HdkTicketsRequest> validator)
        {
        ...
        public async Task<IActionResult> Get([FromQuery] int? id, long? companyId)
        {
            try
            {
                ...
                else
                {
                    ...
                    return Ok(Response<IEnumerable<HdkTicketsReponse>>
                        .Success(
                        companyId.HasValue ? 
                        TicketsTickets.Where(x => x.IdEmpresa == companyId)
                        : TicketsTickets, "Ticket obtenidos correctamente."));
                }
            }
            catch (Exception ex)
            {
